...
public class RenameMethodHandler implements RefactoringHandler {
    @Override
    public ApiSignature mapping(Refactoring refactoring, ApiSignature apiSignature) {
        ...
        UMLOperation renamedOperation = renameOperationRefactoring.getRenamedOperation();
        clone.setClassName(renamedOperation.getClassName());
        clone.setMethodName(renamedOperation.getName());
        List<String> parameterNameList = renamedOperation.getParameterTypeList().stream().map(UMLType::toQualifiedString).toList();
        clone.setMethodParamList(parameterNameList.stream().map(ClassUtil::removeGenericType).toList());
        return clone;
    }
