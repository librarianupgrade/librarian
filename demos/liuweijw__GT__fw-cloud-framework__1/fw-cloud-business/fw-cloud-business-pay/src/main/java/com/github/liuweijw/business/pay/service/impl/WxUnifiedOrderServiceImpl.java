	...
	public R<Map<String, Object>> doWxUnifiedOrderRequest(String tradeType, PayOrder payOrder,
		...
		try {
			...
			if (null == payOrder || null == params || StringHelper.isBlank(tradeType)
					|| StringHelper.isBlank(params.get("resKey")) || StringHelper.isBlank(params.get("channelParam")))
			...
			WxPayConfig wxPayConfig = WxPayUtil.getWxPayConfig(params.get("channelParam"), tradeType,
			...
			try {
				...
				return new R<Map<String, Object>>()
						.data(PayUtil.makeRetData(
								PayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_FAIL,
										"0111", "调用微信支付失败," + e.getErrCode() + ":" + e.getErrCodeDes()),
								resKey))
						.failure();
			}
		} catch (Exception e) {
