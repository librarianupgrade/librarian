	...
	public String notifyTest(@PathVariable String payOrderId) {
		...
		if (payStatus != PayConstant.PAY_STATUS_SUCCESS) {
			...
			if (!updatePayOrderRows) {
				log.error("{}更新支付状态失败,将payOrderId={},更新payStatus={}失败", logPrefix, payOrder.getPayOrderId(),
						PayConstant.PAY_STATUS_SUCCESS);
				return WxPayNotifyResponse.fail("处理订单失败");
			}
			log.error("{}更新支付状态成功,将payOrderId={},更新payStatus={}成功", logPrefix, payOrder.getPayOrderId(),
					PayConstant.PAY_STATUS_SUCCESS);
	...
	public String doNotifyResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!PayConstant.RETURN_VALUE_SUCCESS.equalsIgnoreCase(result.getResultCode())
				...
				log.error("returnCode={},resultCode={},errCode={},errCodeDes={}", result.getReturnCode(),
			...
			if (dbPayAmt != wxPayAmt) {
				log.error("db payOrder record payPrice not equals total_fee. total_fee={},payOrderId={}", total_fee,
						payOrderId);
