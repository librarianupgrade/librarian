...
public class PayChannelServiceImpl extends JPAFactoryImpl implements PayChannelService {
	...
	public PayChannel findPayChannel(String channelId, String mchId) {
		if (StringHelper.isBlank(channelId) || StringHelper.isBlank(mchId))
			return null;

		QPayChannel qPayChannel = QPayChannel.payChannel;
		return this.queryFactory.selectFrom(qPayChannel).where(qPayChannel.channelId.eq(channelId.trim()))
				.where(qPayChannel.mchId.eq(mchId.trim())).fetchFirst();
	}
}
