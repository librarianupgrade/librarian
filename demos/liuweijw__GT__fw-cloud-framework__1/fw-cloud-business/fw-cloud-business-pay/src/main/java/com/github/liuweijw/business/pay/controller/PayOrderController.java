...
public class PayOrderController {
	...
	public R<Map<String, Object>> create(@RequestBody PayUnifiedOrder unifiedOrder) {
		...
		if (!validateResult.getData()) {
			...
			return new R<Map<String, Object>>()
					.data(PayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "订单校验失败", null, null)).failure();
		}

		PayOrder resultOrder = payOrderService.updateOrSavePayOrder(payOrder);
		if (null == resultOrder)
			return new R<Map<String, Object>>()
					.data(PayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "订单保存失败", null, null)).failure();

		switch (resultOrder.getChannelId()) {
		case PayConstant.PAY_CHANNEL_WX_APP:
			return wxUnifiedOrderService.doWxUnifiedOrderRequest(PayConstant.WxConstant.TRADE_TYPE_APP, payOrder,
					params);
		case PayConstant.PAY_CHANNEL_WX_JSAPI:
			return wxUnifiedOrderService.doWxUnifiedOrderRequest(PayConstant.WxConstant.TRADE_TYPE_JSPAI, payOrder,
					params);
		case PayConstant.PAY_CHANNEL_WX_NATIVE:
			return wxUnifiedOrderService.doWxUnifiedOrderRequest(PayConstant.WxConstant.TRADE_TYPE_NATIVE, payOrder,
					params);
		case PayConstant.PAY_CHANNEL_WX_MWEB:
			return wxUnifiedOrderService.doWxUnifiedOrderRequest(PayConstant.WxConstant.TRADE_TYPE_MWEB, payOrder,
					params);
		default:
			break;
		}
		return new R<Map<String, Object>>().data(PayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL,
				"不支持的支付渠道类型[channelId=" + resultOrder.getChannelId() + "]", null, null)).failure();
	}

	@RequestMapping(value = "/find/{mchId}")
	public R<PayOrder> query(@PathVariable String mchId, String payOrderId, String mchOrderNo, Boolean isNotify) {
