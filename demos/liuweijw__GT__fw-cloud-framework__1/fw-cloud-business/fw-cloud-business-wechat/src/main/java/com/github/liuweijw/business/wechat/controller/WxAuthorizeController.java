...
public class WxAuthorizeController {
	...
	public String authorize(HttpServletRequest request, @RequestParam("_backUrl") String _backUrl, Integer from,
			@RequestParam("wechatId") String wechatId, Long t) {
		long start = System.currentTimeMillis();
		log.info("【wxauth.authorize】_backUrl:" + _backUrl);
		from = null == from || from < 0 ? 0 : from;
		t = null == t || t < 0 ? 0 : t;
		String returnUrl = properties.getAuthUrl() + "/wechat/auth/openId?wechatId=" + wechatId + "&from=" + from
				+ "&t=" + t;
		...
		String redirectURL = wxService.oauth2buildAuthorizationUrl(returnUrl,
				from.intValue() == 1 ? WxConsts.OAuth2Scope.SNSAPI_BASE : WxConsts.OAuth2Scope.SNSAPI_USERINFO, state);
	...
	public String openId(HttpServletRequest request, @RequestParam("code") String code,
		...
		String redirectUrl = RequestUtil.buildAppendURLParams(RequestUtil.buildURLParams(returnUrl, OPENID),
				OPENID + "=" + openId, "t=" + t);
