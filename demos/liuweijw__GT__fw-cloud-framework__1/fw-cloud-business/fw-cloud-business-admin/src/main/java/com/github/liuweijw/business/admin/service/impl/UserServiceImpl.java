...
public class UserServiceImpl extends JPAFactoryImpl implements UserService {
	...
	public List<Role> findRoleListByUserId(Integer userId) {
		...
		List<Role> rList = this.queryFactory.select(qRole).from(qUserRole, qRole).where(qUserRole.userId.eq(userId))
				.where(qUserRole.roleId.eq(qRole.roleId)).fetch();
	...
	public PageBean<User> findAll(PageParams pageParams, User user) {
		...
		querySql.append("select ").append(
				"t1.user_id, t1.username, t1.open_id, t1.mobile,t1.pic_url,t1.dept_id, t1.create_time, t1.update_time, t1.statu,")
				.append("t3.dept_name as deptName ").append("from " + User.TABLE_NAME + " t1 ")
		...
		if (StringHelper.isNotBlank(user.getUsername())) {
			whereSql.append("and t1.username like ").append("'%" + user.getUsername().trim() + "%' escape '!' ");
		}
