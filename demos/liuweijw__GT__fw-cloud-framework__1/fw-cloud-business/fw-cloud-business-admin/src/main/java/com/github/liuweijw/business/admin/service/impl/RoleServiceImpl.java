...
public class RoleServiceImpl extends JPAFactoryImpl implements RoleService {
	...
	public List<Role> findRoleListByDeptId(Integer deptId) {
		...
		QRole qRole = QRole.role;
		List<Role> rList = this.queryFactory.select(qRole).from(qRoleDept, qRole).where(qRoleDept.deptId.eq(deptId))
				.where(qRoleDept.roleId.eq(qRole.roleId)).fetch();
	...
	public PageBean<RoleDeptBean> findAll(PageParams pageParams, Role role) {
		...
		querySql.append("select ").append(
				"t1.role_id, t1.role_name, t1.role_code, t1.role_desc, t1.create_time, t1.update_time, t1.statu,")
				.append("t3.dept_id, t3.pid, t3.dept_name ").append("from " + Role.TABLE_NAME + " t1 ")
		...
		if (StringHelper.isNotBlank(role.getRoleName())) {
			whereSql.append("and t1.role_name like ").append("'%" + role.getRoleName().trim() + "%' escape '!' ");
		}

		if (StringHelper.isNotBlank(role.getRoleCode())) {
			whereSql.append("and t1.role_code like ").append("'%" + role.getRoleCode().trim() + "%' escape '!' ");
		}
