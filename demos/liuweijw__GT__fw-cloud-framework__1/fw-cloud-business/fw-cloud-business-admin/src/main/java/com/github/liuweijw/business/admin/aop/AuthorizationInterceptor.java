...
public class AuthorizationInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (!clazz.isAnnotationPresent(PrePermissions.class)) {
			...
			R<Boolean> responseWithR = new R<Boolean>()
		...
		if (!method.isAnnotationPresent(PrePermissions.class)) {
			...
			R<Boolean> responseWithR = new R<Boolean>()
					.failure("请求[" + requestURI + "]方法上未设置权限,请设置注解@PrePermissions权限！").data(false);
			this.handleWithResponse(response, responseWithR);
			return false;
		}
		...
		if (null == permissions || permissions.length == 0) {
			...
			R<Boolean> responseWithR = new R<Boolean>()
					.failure("请求[" + requestURI + "]方法上未正确设置权限,请设置注解@PrePermissions权限！").data(false);
			this.handleWithResponse(response, responseWithR);
			return false;
		}
		...
		for (String permission : permissions) {
			...
			if (!menuPermissions.contains(valiatePermission)) {
				...
				R<Boolean> responseWithR = new R<Boolean>()
						.failure("请求[" + requestURI + "]权限[" + valiatePermission + "]未配置！").data(false);
				this.handleWithResponse(response, responseWithR);
				return false;
			}
		}
	...
	public void handleWithResponse(HttpServletResponse response, R<Boolean> responseWithR) {
		try {
			RequestUtil.handleWithResponse(response, this.objectMapper.writeValueAsString(responseWithR));
		} catch (IOException e) {
			e.printStackTrace();
