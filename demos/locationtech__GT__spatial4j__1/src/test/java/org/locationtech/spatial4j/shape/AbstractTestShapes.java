...
public abstract class AbstractTestShapes extends RandomizedShapeTest {
		...
		for (double left = -180; left <= 180; left += INCR) {
			for (double right = left; right - left <= 360; right += INCR) {
				...
				for (double left2 = right + INCR; left2 - left < 360; left2 += INCR) {
					//test point disjoint
					assertRelation(null, SpatialRelation.DISJOINT, r,
							ctx.makePoint(normX(left2), randomIntBetween(-90, 90)));
		...
		new RectIntersectionTestHelper<Circle>(ctx) {
			...
			protected void onAssertFail(AssertionError e, Circle s, Rectangle r, SpatialRelation ic) {
				...
				if (radius == 0) {
					...
					if (p.getX() == r.getMinX() || p.getX() == r.getMaxX() || p.getY() == r.getMinY()
							|| p.getY() == r.getMaxY())
						return;
					throw e;
				}
				...
				System.out.println("Seed " + getContext().getRunnerSeedAsString()
						+ ": Hid assertion due to ambiguous edge touch: " + s + " " + r);
			}
	...
	public void testMakeRect() {
		...
		//test ctx.makeRect
		assertEquals(ctx.makeRectangle(1, 3, 2, 4), ctx.makeRectangle(ctx.makePoint(1, 2), ctx.makePoint(3, 4)));
	}
