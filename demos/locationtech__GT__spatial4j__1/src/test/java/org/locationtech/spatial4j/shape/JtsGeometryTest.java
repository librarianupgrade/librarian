...
public class JtsGeometryTest extends AbstractTestShapes {
	...
	public void testWidthGreaterThan180() throws ParseException {
		...
		JtsGeometry jtsGeo = (JtsGeometry) wkt(ctx,
	...
	public void testFiji() throws IOException, ParseException {
		...
		assertRelation(null, SpatialRelation.CONTAINS, shape, ctx.makePoint(-179.99, -16.9));
		assertRelation(null, SpatialRelation.CONTAINS, shape, ctx.makePoint(+179.99, -16.9));
	...
	public void testNarrowGeometryCollection() {
		...
		// test lines
		gcol = Geom.build().point(1, 1).point(2, 2).lineString().point(3, 3).point(4, 4).lineString().toCollection();

		geom = JtsSpatialContext.GEO.makeShape(gcol);
		assertTrue(geom.getGeom() instanceof MultiLineString);

		// test polygons
		gcol = Geom.build().point(1, 1).point().buffer(1).point(2, 3).point().buffer(1).toCollection();

		geom = JtsSpatialContext.GEO.makeShape(gcol);
		assertTrue(geom.getGeom() instanceof MultiPolygon);

		// test heterogenous
		gcol = Geom.build().point(0, 0).point().point(1, 1).point(2, 2).lineString().toCollection();
		try {
