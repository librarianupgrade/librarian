...
public class TestDistances extends RandomizedTest {
	...
	public void testDistancesAgainstVincenty() {
		...
		for (int i = 0; i < TRIES; i++) {
			...
			assertEquals(distV, havV,
					(distV <= 90) ? DistanceUtils.KM_TO_DEG * 0.00001 : DistanceUtils.KM_TO_DEG * 0.001);
	...
	private Point randomGeoPointFrom(Point p1) {
		...
		Point p2RAD = DistanceUtils.pointOnBearingRAD(DistanceUtils.toRadians(p1.getY()),
				DistanceUtils.toRadians(p1.getX()), DistanceUtils.toRadians(distDEG),
				DistanceUtils.toRadians(bearingDEG), ctx, null);
	...
	public void testHaversineNaN() {
		assertEquals(180, new GeodesicSphereDistCalc.Haversine().distance(
				ctx.makePoint(-81.05206968336057, 71.82629271026536), 98.9479297952497, -71.82629264390964), 0.00001);
	}
	...
	public void testCalcBoxByDistFromPt() {
		...
		assertEquals("0 dist, horiz line", -45,
				dc().calcBoxByDistFromPt_yHorizAxisDEG(ctx.makePoint(-180, -45), 0, ctx), 0);
	...
	public void testNormLat() {
		double[][] lats = new double[][] { { 1.23, 1.23 }, //1.23 might become 1.2299999 after some math and we want to ensure that doesn't happen
				{ -90, -90 }, { 90, 90 }, { 0, 0 }, { -100, -80 }, { -90 - 180, 90 }, { -90 - 360, -90 },
				{ 90 + 180, -90 }, { 90 + 360, 90 }, { -12 + 180, 12 } };
	...
	public void testNormLon() {
		double[][] lons = new double[][] { { 1.23, 1.23 }, //1.23 might become 1.2299999 after some math and we want to ensure that doesn't happen
