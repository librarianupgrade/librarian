...
public abstract class RectIntersectionTestHelper<S extends Shape> extends RandomizedShapeTest {
	...
	public void testRelateWithRectangle() {
		...
		while (i_C < getContainsMinimum(MINLAPS) || i_I < getIntersectsMinimum(MINLAPS)
				|| i_W < getWithinMinimum(MINLAPS) || (!isRandomShapeRectangular() && i_D < getDisjointMinimum(MINLAPS))
			...
			if (laps > MINLAPS) {
				fail("Did not find enough contains/within/intersection/disjoint/bounds cases in a reasonable number"
						+ " of random attempts. CWIDbD: " + i_C + "(" + getContainsMinimum(MINLAPS) + ")," + i_W + "("
						+ getWithinMinimum(MINLAPS) + ")," + i_I + "(" + getIntersectsMinimum(MINLAPS) + ")," + i_D
						+ "(" + getDisjointMinimum(MINLAPS) + ")," + i_bboxD + "(" + getBoundingMinimum(MINLAPS) + ")"
