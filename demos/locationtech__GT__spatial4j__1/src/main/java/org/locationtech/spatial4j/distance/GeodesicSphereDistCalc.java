...
public abstract class GeodesicSphereDistCalc extends AbstractDistanceCalculator {
	...
	public Point pointOnBearing(Point from, double distDEG, double bearingDEG, SpatialContext ctx, Point reuse) {
		...
		Point result = DistanceUtils.pointOnBearingRAD(toRadians(from.getY()), toRadians(from.getX()),
				toRadians(distDEG), toRadians(bearingDEG), ctx, reuse);//output result is in radians
	...
	public final double distance(Point from, double toX, double toY) {
		return toDegrees(
				distanceLatLonRAD(toRadians(from.getY()), toRadians(from.getX()), toRadians(toY), toRadians(toX)));
	}
